...
# ARCHITECTURE.md. This document serves:
#
# - Pre-mortem analysis
...
**Source Decision:** Use of next-auth with HTTP-only cookies (ARCHITECTURE.md §7.2, §9)
...
### Trigger Condition
Agent-generated `auth.config.ts` includes `accessToken` in the `session()` callback without explicit instruction to keep it server-only, OR a developer manually adds it to debug FHIR calls from the browser.
...
### Mitigation
1. **Strict code review gate:** The `session()` callback in `lib/auth/auth.config.ts` must NOT expose `accessToken` to the client.
2. **Server-side FHIR fetching only:** All FHIR data retrieval happens in React Server Components or Next.js API routes — never in client components with a raw token.
3. **Automated test:** Playwright test asserts `window.__NEXT_DATA__` does not contain an access token pattern.
4. **ARCHITECTURE.md already specifies:** "OAuth2 token never exposed to browser JavaScript (HTTP-only session cookie)" — §5 Non-Functional Goals > Security. Agents must not violate this.
...
**Source Decision:** Framework choice (Next.js) interacting with legacy PHP backend (ARCHITECTURE.md §6, §10.1)
...
### Mitigation
1. **Proxy layer pre-built:** `app/api/fhir/[...resource]/route.ts` is part of the standard directory structure (ARCHITECTURE.md §18.1). It is not an emergency add-on.
...
**Source Decision:** Consuming OpenEMR's FHIR API as an immutable contract (ARCHITECTURE.md §10.1, §18.4)
...
**Source Decision:** AI-First development with Cursor agent generation (ARCHITECTURE.md §7, §12)
...
**Source Decision:** React Query with 30-second stale-while-revalidate TTL (ARCHITECTURE.md §5, §16)
...
**Source Decision:** Deployment on Vercel (serverless) vs Railway (always-on container) (ARCHITECTURE.md §15, §16)
...
**Source Decision:** Human-in-the-loop accountability model (ARCHITECTURE.md §7, §8)

### Description
Under time pressure (four-day delivery), the developer may skip the Agent Council Review steps defined in ARCHITECTURE.md §8 and deploy AI-generated code without adversarial review, security analysis, or verification against the PHP dashboard. This undermines the entire accountability model.
...
**Source Decision:** External platform dependency for the new frontend (ARCHITECTURE.md §15)
...
**Source Decision:** Use of next-auth with default JWT strategy (stateless) (ARCHITECTURE.md §7.2)
...
2. **Document this as a scope augmentation:** The ARCHITECTURE.md clarifies that patient navigation is a necessary shell for demonstrating the dashboard, not a redesign of the patient search workflow.
...
- [ ] **R-001:** `auth.config.ts` reviewed — no `accessToken` in client session callback. Playwright test confirms no token in `window` object.
...
| ---- | ------------------------------------------------------ | -------------------------------------------- |
| R-001 | Playwright security test in CI                        | Test fails → block merge                     |
...
*This document is part of the Echelon Enterprise Engineering governance suite. It is traceable to ARCHITECTURE_PRD2_MODERNIZED.md §1-20 and serves as the primary input to the Agent Council Review process.*
```